...
[![version](https://img.shields.io/badge/version-1.0.0-blue)](#status) [![platform](https://img.shields.io/badge/Claude%20Code-Windows%20%7C%20macOS%20%7C%20Linux-informational)](#support-matrix) [![license](https://img.shields.io/badge/license-MIT-blue)](#license)
...
- **Background fan-out runs unmonitored.** If you launch work that burns tokens *away* from the main conversation — a background workflow, a large parallel subagent fan-out — Brink does not see it. Its usage reading comes from the statusline, which only refreshes on main-session activity, and its gate only runs on main-session tool calls. While the main session sits idle waiting, both are frozen. Observed on 2026-07-22: a background fan-out spent ~5M tokens across 104 agents in ~15 minutes and hit the session limit with **no warning and no pause** from Brink. Burn-rate projection does not help here, because no fresh readings arrive to project from. Treat heavy background work as outside Brink's coverage until this is closed.
